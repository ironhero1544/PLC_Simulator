...
bool CreateZipFileWindows(const std::string& zipPath, const std::map<std::string, std::string>& files) {
    // 1. 임시 디렉토리 생성
    ...
    DWORD tempResult = GetShortPathNameA(tempPath.c_str(), shortTempPath, MAX_PATH);
    
    // 짧은 경로명 변환이 실패하면 원본 경로 사용
    std::string finalZipPath = (zipResult > 0) ? std::string(shortZipPath) : zipPath;
    std::string finalTempPath = (tempResult > 0) ? std::string(shortTempPath) : tempPath;
    ...
    std::string finalCmd = "powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command \"& {";
...
bool ExtractZipFileWindows(const std::string& zipPath, std::map<std::string, std::string>& files) {
    // 1. 임시 추출 디렉토리
    char tempDir[MAX_PATH];
    GetTempPathA(MAX_PATH, tempDir);
    ...
    DWORD zipResult = GetShortPathNameA(zipPath.c_str(), shortZipPath, MAX_PATH);
    DWORD extractResult = GetShortPathNameA(extractPath.c_str(), shortExtractPath, MAX_PATH);
    
    // 짧은 경로명 변환이 실패하면 원본 경로 사용
    std::string finalZipPath = (zipResult > 0) ? std::string(shortZipPath) : zipPath;
    std::string finalExtractPath = (extractResult > 0) ? std::string(shortExtractPath) : extractPath;
    
    // 3. PowerShell로 압축 해제 (더 안전한 방식)
    std::string powershellCmd = "powershell.exe -ExecutionPolicy Bypass -Command \"& {";
    powershellCmd += "Add-Type -AssemblyName System.IO.Compression.FileSystem; ";
    powershellCmd += "try { [System.IO.Compression.ZipFile]::ExtractToDirectory('";
        ...
        if (hFind != INVALID_HANDLE_VALUE) {
            ...
            do {
                if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                    ...
                    printf("[DEBUG]   - %s (%lu bytes)\n", fileName.c_str(), findData.nFileSizeLow);
...
ProjectFileManager::ProjectFileManager() {
    m_xmlSerializer = std::make_unique<XMLSerializer>();
...
ProjectFileManager::SaveResult ProjectFileManager::SaveProject(const LadderProgram& program, 
    ...
    try {
        ...
        m_xmlSerializer->SetDebugMode(m_debugMode);
        auto xmlResult = m_xmlSerializer->SerializeToXML(program);
        if (!xmlResult.success) {
            result.success = false;
            result.errorMessage = "Failed to serialize ladder program: " + xmlResult.errorMessage;
            SetError(result.errorMessage);
            return result;
        }
        
        // 2. LD 프로그램 생성 (실행용)
        m_ldConverter->SetDebugMode(m_debugMode);
        std::string ldContent = m_ldConverter->ConvertToLDString(program);
        if (ldContent.empty()) {
            result.success = false;
            result.errorMessage = "Failed to convert ladder to LD format: " + m_ldConverter->GetLastError();
            SetError(result.errorMessage);
            return result;
        }
        ...
        // 3. 프로젝트 정보 생성
        ProjectInfo info;
        info.projectName = projectName.empty() ? ExtractFileName(filePath) : projectName;
        info.version = "1.0";
        ...
        if (!CreateZipFile(filePath, files)) {
            result.success = false;
            result.errorMessage = "Failed to create ZIP file: " + m_lastError;
            return result;
        }
        ...
        LogDebug("✅ Project saved successfully: " + std::to_string(result.compressedSize) + " bytes");
...
ProjectFileManager::LoadResult ProjectFileManager::LoadProject(const std::string& filePath) {
    LoadResult result;
    m_lastError.clear();
    ...
    try {
        ...
        if (!ExtractZipFile(filePath, files)) {
            result.success = false;
            result.errorMessage = "Failed to extract ZIP file: " + m_lastError;
            return result;
        }
        ...
        // 4. XML 역직렬화 (UI 구조 복원)
        m_xmlSerializer->SetDebugMode(m_debugMode);
        auto xmlResult = m_xmlSerializer->DeserializeFromXML(files["layout.xml"]);
        if (!xmlResult.success) {
            result.success = false;
            result.errorMessage = "Failed to deserialize ladder program: " + xmlResult.errorMessage;
            SetError(result.errorMessage);
            return result;
        }
        ...
        LogDebug("✅ Project loaded successfully: " + std::to_string(result.program.rungs.size()) + 
...
ProjectFileManager::ProjectInfo ProjectFileManager::GetProjectInfo(const std::string& filePath) {
    ProjectInfo info;
...
bool ProjectFileManager::ValidateProjectFile(const std::string& filePath) {
    try {
        ...
        // XML 유효성 검사
        m_xmlSerializer->SetDebugMode(false); // 검증 시에는 조용히
        auto xmlResult = m_xmlSerializer->DeserializeFromXML(files["layout.xml"]);
...
bool ProjectFileManager::CreateZipFile(const std::string& filePath, 
...
#else
    // 다른 플랫폼에서는 원래 miniz 사용 (Windows 전용 프로젝트이므로 사용되지 않음)
    SetError("Windows-only project: Shell API not available on this platform");
    return false;
#endif
}

bool ProjectFileManager::ExtractZipFile(const std::string& filePath, 
                                       std::map<std::string, std::string>& files) {
    ...
    if (ExtractZipFileWindows(filePath, files)) {
        ...
        for (const auto& [fileName, content] : files) {
            LogDebug("   - " + fileName + " (" + std::to_string(content.size()) + " bytes)");
        }
        return true;
    } else {
...
std::string ProjectFileManager::GenerateLDProgram(const LadderProgram& program) {
    return m_ldConverter->ConvertToLDString(program);
}

bool ProjectFileManager::ParseManifest(const std::string& manifestContent, ProjectInfo& info) {
    try {
...
std::string ProjectFileManager::CreateSimpleJSON(const std::map<std::string, std::string>& keyValues) {
...
std::map<std::string, std::string> ProjectFileManager::ParseSimpleJSON(const std::string& jsonContent) {
