...
XMLSerializer::XMLSerializer() : m_debugMode(false) {
}

XMLSerializer::~XMLSerializer() {
}

XMLSerializer::SerializationResult XMLSerializer::SerializeToXML(const LadderProgram& program) {
    ...
    try {
        ...
        LogDebug("✅ XML serialization completed: " + std::to_string(result.bytesWritten) + " bytes");
...
XMLSerializer::DeserializationResult XMLSerializer::DeserializeFromXML(const std::string& xmlContent) {
    ...
    try {
        ...
        if (!ParseXMLHeader(xmlContent)) {
            result.success = false;
            result.errorMessage = "Invalid XML header: " + m_lastError;
            return result;
        }
        ...
        if (!ParseMetadata(xmlContent, result)) {
            result.success = false;
            result.errorMessage = "Failed to parse metadata: " + m_lastError;
            return result;
        }
        ...
        if (!ParseRungs(xmlContent, result.program)) {
            result.success = false;
            result.errorMessage = "Failed to parse rungs: " + m_lastError;
            return result;
        }
        ...
        if (!ParseVerticalConnections(xmlContent, result.program)) {
            result.success = false;
            result.errorMessage = "Failed to parse vertical connections: " + m_lastError;
            return result;
        }
        
        result.success = true;
        LogDebug("✅ XML deserialization completed: " + std::to_string(result.program.rungs.size()) + 
                 " rungs, " + std::to_string(result.program.verticalConnections.size()) + " connections");
        
    } catch (const std::exception& e) {
        result.success = false;
        result.errorMessage = "XML deserialization failed: " + std::string(e.what());
        SetError(result.errorMessage);
    }
...
bool XMLSerializer::SaveToXMLFile(const LadderProgram& program, const std::string& filePath) {
    ...
    LogDebug("💾 Saved XML to file: " + filePath + " (" + std::to_string(result.bytesWritten) + " bytes)");
    return true;
}

XMLSerializer::DeserializationResult XMLSerializer::LoadFromXMLFile(const std::string& filePath) {
    DeserializationResult result;
    ...
    LogDebug("📖 Loaded XML from file: " + filePath + " (" + std::to_string(buffer.str().length()) + " bytes)");
...
std::string XMLSerializer::GenerateMetadata() const {
    ...
    meta << "    <Description>Complete ladder program structure with UI layout preservation</Description>\n";
    meta << "  </Metadata>\n";
    return meta.str();
}

std::string XMLSerializer::SerializeRungs(const std::vector<Rung>& rungs) const {
    std::stringstream xml;
...
std::string XMLSerializer::SerializeRung(const Rung& rung, int index) const {
    std::stringstream xml;
    xml << "    <Rung index=\"" << index << "\" number=\"" << rung.number << "\" isEndRung=\"" << (rung.isEndRung ? "true" : "false") << "\">\n";
...
std::string XMLSerializer::SerializeCell(const LadderInstruction& cell, int rungIndex, int cellIndex) const {
    std::stringstream xml;
    xml << "        <Cell index=\"" << cellIndex << "\" rung=\"" << rungIndex << "\">\n";
    xml << "          <Type>" << InstructionTypeToString(cell.type) << "</Type>\n";
    xml << "          <Address>" << EscapeXMLString(cell.address) << "</Address>\n";
    xml << "          <Preset>" << EscapeXMLString(cell.preset) << "</Preset>\n";
    xml << "          <IsActive>" << (cell.isActive ? "true" : "false") << "</IsActive>\n";
    xml << "        </Cell>\n";
    return xml.str();
}

std::string XMLSerializer::SerializeVerticalConnections(const std::vector<VerticalConnection>& connections) const {
    std::stringstream xml;
...
std::string XMLSerializer::SerializeVerticalConnection(const VerticalConnection& conn, int index) const {
    std::stringstream xml;
...
bool XMLSerializer::ParseMetadata(const std::string& xmlContent, DeserializationResult& result) {
    ...
    LogDebug("📄 Metadata parsed - Version: " + result.sourceVersion + ", Date: " + result.createdDate);
    return true;
}

bool XMLSerializer::ParseRungs(const std::string& xmlContent, LadderProgram& program) {
...
bool XMLSerializer::ParseCell(const std::string& cellXML, LadderInstruction& cell) {
    std::string typeStr = GetXMLContent(ExtractXMLTag(cellXML, "Type"));
    cell.type = StringToInstructionType(typeStr);
    cell.address = UnescapeXMLString(GetXMLContent(ExtractXMLTag(cellXML, "Address")));
    cell.preset = UnescapeXMLString(GetXMLContent(ExtractXMLTag(cellXML, "Preset")));
    cell.isActive = (GetXMLContent(ExtractXMLTag(cellXML, "IsActive")) == "true");
    
    return true;
}

bool XMLSerializer::ParseVerticalConnections(const std::string& xmlContent, LadderProgram& program) {
    std::string connectionsSection = ExtractXMLTag(xmlContent, "VerticalConnections");
    ...
    LogDebug("🔗 Parsed " + std::to_string(program.verticalConnections.size()) + " vertical connections");
    return true;
}

bool XMLSerializer::ParseVerticalConnection(const std::string& connXML, VerticalConnection& conn) {
...
std::string XMLSerializer::EscapeXMLString(const std::string& str) const {
    ...
    std::regex amp("&"); result = std::regex_replace(result, amp, "&amp;");
    std::regex lt("<"); result = std::regex_replace(result, lt, "&lt;");
    std::regex gt(">"); result = std::regex_replace(result, gt, "&gt;");
    std::regex quot("\""); result = std::regex_replace(result, quot, "&quot;");
    std::regex apos("'"); result = std::regex_replace(result, apos, "&apos;");
    return result;
}
...
std::string XMLSerializer::InstructionTypeToString(LadderInstructionType type) const {
    switch (type) {
        case LadderInstructionType::EMPTY: return "EMPTY";
...
LadderInstructionType XMLSerializer::StringToInstructionType(const std::string& str) const {
    if (str == "EMPTY") return LadderInstructionType::EMPTY;
...
std::string XMLSerializer::ExtractXMLTag(const std::string& xml, const std::string& tagName) const {
...
std::vector<std::string> XMLSerializer::ExtractXMLTags(const std::string& xml, const std::string& tagName) const {
...
std::string XMLSerializer::GetXMLAttribute(const std::string& xmlTag, const std::string& attrName) const {
