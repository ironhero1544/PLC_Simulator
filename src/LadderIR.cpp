...
#include "LadderIR.h"
#include "ProgrammingMode.h"  // 실제 LadderProgram, LadderRung 등 타입 정의
#include <iostream>
#include <sstream>
#include <algorithm>
#include <queue>
...
std::string IRNode::GetDisplayName() const {
    switch (nodeType) {
        case IRNodeType::NORMALLY_OPEN:     return "NO[" + deviceAddress + "]";
        case IRNodeType::NORMALLY_CLOSED:   return "NC[" + deviceAddress + "]";
        case IRNodeType::OUTPUT_COIL:       return "OUT[" + deviceAddress + "]";
        case IRNodeType::SET_COIL:          return "SET[" + deviceAddress + "]";
        case IRNodeType::RESET_COIL:        return "RST[" + deviceAddress + "]";
        case IRNodeType::AND_BLOCK:         return "AND_BLOCK";
        case IRNodeType::OR_BLOCK:          return "OR_BLOCK";
        case IRNodeType::TIMER_ON:          return "TON[" + deviceAddress + "]";
        case IRNodeType::TIMER_OFF:         return "TOF[" + deviceAddress + "]";
        case IRNodeType::COUNTER_UP:        return "CTU[" + deviceAddress + "]";
        case IRNodeType::COUNTER_DOWN:      return "CTD[" + deviceAddress + "]";
        case IRNodeType::EMPTY:             return "EMPTY";
        case IRNodeType::VERTICAL_LINE:     return "VLINE";
        case IRNodeType::HORIZONTAL_LINE:   return "HLINE";
...
bool IRNode::IsOutputNode() const {
    return nodeType == IRNodeType::OUTPUT_COIL ||
           nodeType == IRNodeType::SET_COIL ||
           nodeType == IRNodeType::RESET_COIL;
}

bool IRNode::IsLogicNode() const {
    return nodeType == IRNodeType::AND_BLOCK ||
           nodeType == IRNodeType::OR_BLOCK;
}
...
void IRRung::AddConnection(size_t fromId, size_t toId, int outPort, int inPort) {
...
void IRRung::AnalyzeParallelConnections() {
    ...
    for (const auto& node : nodes) {
        if (node->IsInputNode() && !node->inputs.empty() && !node->outputs.empty()) {
    ...
    for (const auto& [inputOutput, nodeGroup] : parallelGroups) {
        if (nodeGroup.size() >= 2) {
            // OR 블록 노드 생성 (다음 구현에서...)
            std::cout << "[IR] Found parallel group: " << nodeGroup.size() << " nodes\n";
        }
    }
}
...
void LadderIRProgram::AddRung(IRRung&& rung) {
    m_rungs.push_back(std::move(rung));
}

void LadderIRProgram::MapDeviceToNode(const std::string& deviceAddr, size_t nodeId) {
    m_deviceNodeMap[deviceAddr] = nodeId;
}

size_t LadderIRProgram::FindNodeByDevice(const std::string& deviceAddr) const {
    auto it = m_deviceNodeMap.find(deviceAddr);
    return (it != m_deviceNodeMap.end()) ? it->second : 0;
}

void LadderIRProgram::AnalyzeLogicStructure() {
    std::cout << "[IR] Analyzing logic structure for " << m_rungs.size() << " rungs...\n";
    
    for (auto& rung : m_rungs) {
...
void LadderIRProgram::OptimizeConnections() {
    ...
    for (auto& rung : m_rungs) {
        // 빈 노드들 제거
        rung.nodes.erase(
            std::remove_if(rung.nodes.begin(), rung.nodes.end(),
                [](const std::unique_ptr<IRNode>& node) {
                    return node->nodeType == IRNodeType::EMPTY;
                }),
            rung.nodes.end()
        );
    }
...
void LadderIRProgram::PrintIRStructure() const {
    std::cout << "\n=== LadderIR Structure ===\n";
    std::cout << "Total Rungs: " << m_rungs.size() << "\n\n";
    
    for (size_t i = 0; i < m_rungs.size(); ++i) {
...
LadderIRProgram LadderToIRConverter::ConvertToIR(const LadderProgram& ladderProgram) {
    ...
    for (size_t i = 0; i < ladderProgram.rungs.size(); ++i) {
        ...
        std::cout << "[IR] Converting rung " << i << " with " << rung.cells.size() << " cells\n";
...
IRRung LadderToIRConverter::ConvertRungToIR(const Rung& rung, LadderIRProgram& irProgram) {
    IRRung irRung;
    ...
    for (size_t j = 0; j < rung.cells.size(); ++j) {
        ...
        if (cell.type != LadderInstructionType::EMPTY) {
            ...
            auto node = ConvertCellToNode(cell, nodeId, static_cast<int>(0), static_cast<int>(j));
            if (node) {
                // 디바이스 매핑
...
std::unique_ptr<IRNode> LadderToIRConverter::ConvertCellToNode(
    ...
    switch (cell.type) {
        case LadderInstructionType::XIC:
            node = std::make_unique<IRNode>(IRNodeType::NORMALLY_OPEN, nodeId, cell.address);
            break;
            
        case LadderInstructionType::XIO:
            node = std::make_unique<IRNode>(IRNodeType::NORMALLY_CLOSED, nodeId, cell.address);
            break;
            
        case LadderInstructionType::OTE:
            node = std::make_unique<IRNode>(IRNodeType::OUTPUT_COIL, nodeId, cell.address);
            break;
            
        case LadderInstructionType::SET:
            node = std::make_unique<IRNode>(IRNodeType::SET_COIL, nodeId, cell.address);
            break;
            
        case LadderInstructionType::RST:
            node = std::make_unique<IRNode>(IRNodeType::RESET_COIL, nodeId, cell.address);
            break;
            
        case LadderInstructionType::TON:
            {
                auto timerNode = std::make_unique<IRTimerNode>(nodeId, cell.address, 1.0f);
                ...
                if (!cell.preset.empty()) {
                    ...
                    if (cell.preset[0] == 'K') {
                        try {
                            ...
                            timerNode->timeValue = static_cast<float>(timerValue) * 0.1f; // 100ms 단위
                        } catch (...) {
                            std::cout << "[IR] Warning: Invalid timer preset: " 
                                      << cell.preset << std::endl;
                        }
                    }
                }
        ...
        case LadderInstructionType::CTU:
            {
                auto counterNode = std::make_unique<IRCounterNode>(nodeId, cell.address, 1);
                ...
                if (!cell.preset.empty()) {
                    if (cell.preset[0] == 'K') {
                        try {
                            ...
                            std::cout << "[IR] Warning: Invalid counter preset: " 
                                      << cell.preset << std::endl;
                        }
                    }
                }
...
void LadderToIRConverter::AnalyzeVerticalConnections(
    ...
    std::cout << "[IR] Analyzing " << verticalConnections.size() << " vertical connections...\n";
    ...
    for (const auto& vconn : verticalConnections) {
        ...
        if (parallelNodes.size() >= 2) {
            std::cout << "[IR] Creating OR block with " << parallelNodes.size() << " nodes\n";
        }
    }
}
...
void LadderToIRConverter::AnalyzeLogicFlow(IRRung& irRung) {
    ...
    std::cout << "[IR] Analyzing logic flow for rung with " << irRung.nodes.size() << " nodes\n";
...
std::vector<std::string> IRToStackConverter::ConvertToStackInstructions(
    ...
    std::cout << "[IR] Generated " << allInstructions.size() << " stack instructions\n";
    return allInstructions;
}

std::vector<std::string> IRToStackConverter::ConvertRungToStack(const IRRung& rung) {
    ...
    for (const auto& node : rung.nodes) {
        if (node->IsInputNode()) {
            ...
            // 🔥 **고급 타이머 명령어 생성**
            const IRTimerNode* timerNode = dynamic_cast<const IRTimerNode*>(node.get());
            if (timerNode) {
            ...
            // 🔥 **고급 카운터 명령어 생성**
            const IRCounterNode* counterNode = dynamic_cast<const IRCounterNode*>(node.get());
            if (counterNode) {
...
std::vector<std::string> IRToStackConverter::ConvertOrBlockToStack(const IROrBlockNode& orBlock) {
    std::vector<std::string> instructions;
...
void IRToStackConverter::OptimizeStackInstructions(std::vector<std::string>& instructions) {
    // 스택 명령어 최적화
    // 1. 연속된 LOAD/AND 최적화
    // 2. 불필요한 MPS/MPP 제거
    // 3. 중복 명령어 제거
    
    std::cout << "[IR] Optimizing " << instructions.size() << " stack instructions...\n";
    
    // 현재는 기본 최적화만 구현 (빈 줄 제거)
    instructions.erase(
        std::remove_if(instructions.begin(), instructions.end(),
            [](const std::string& instr) {
                return instr.empty() || instr.find("//") == 0;
            }),
        instructions.end()
    );
