...
// 행렬-벡터 곱셈: y = A*x
void MatrixVectorMultiply(float** A, const float* x, float* y, int rows, int cols) {
...
public:
    ...
    static int SolveElectricalNetwork(ElectricalNetwork* network, float deltaTime) {
        (void)deltaTime; // 현재 버전에서는 정적 상태 해석만 수행, 나중에 동적 해석에서 사용할 예정
        if (!network || network->nodeCount == 0) return -1;
        ...
        while (network->currentIteration < network->maxIterations && !network->isConverged) {
            // 가우스-자이델 스위프
            for (int i = 0; i < network->nodeCount; i++) {
                ...
                if (std::abs(network->conductanceMatrix[i][i]) > 1e-12f) {
                    network->voltageVector[i] = (network->currentVector[i] - sum) / network->conductanceMatrix[i][i];
                } else {
...
private:
    ...
    static void UpdateConductanceMatrix(ElectricalNetwork* network) {
        ...
        for (int e = 0; e < network->edgeCount; e++) {
            ...
            if (!edge->base.isActive) continue;
            
            int i = edge->base.fromNodeId;
            int j = edge->base.toNodeId;
            float g = edge->conductance;
            
            if (i >= 0 && i < network->nodeCount && j >= 0 && j < network->nodeCount) {
                // 비대각항: G_ij = -g_ij
    ...
    static void UpdateCurrentVector(ElectricalNetwork* network) {
        for (int i = 0; i < network->nodeCount; i++) {
            ...
            if (network->nodes[i].isVoltageSource && network->nodes[i].sourceResistance > 0) {
                float g_internal = 1.0f / network->nodes[i].sourceResistance;
                network->currentVector[i] += g_internal * network->nodes[i].sourceVoltage;
    ...
    static void CalculateEdgeCurrents(ElectricalNetwork* network) {
        for (int e = 0; e < network->edgeCount; e++) {
            ...
            if (fromNode >= 0 && fromNode < network->nodeCount && 
                toNode >= 0 && toNode < network->nodeCount) {
                
                float voltageDrop = network->voltageVector[fromNode] - network->voltageVector[toNode];
                edge->current = voltageDrop * edge->conductance;
                edge->voltageDrop = voltageDrop;
                edge->powerDissipation = edge->current * edge->current * edge->resistance;
    ...
    static void CalculateNodeCurrents(ElectricalNetwork* network) {
        for (int i = 0; i < network->nodeCount; i++) {
            ...
            // 연결된 모든 엣지의 전류를 합산
            for (int e = 0; e < network->edgeCount; e++) {
                if (!network->edges[e].base.isActive) continue;
...
public:
    ...
    static int SolvePneumaticNetwork(PneumaticNetwork* network, float deltaTime) {
        ...
        while (network->currentIteration < network->maxIterations && !network->isConverged) {
            ...
            // 선형 시스템 해결: J * Δx = -F
            SolveLinearSystem(jacobian, residual, delta, network->nodeCount + network->edgeCount);
            
            // 해 업데이트
            UpdateSolution(network, delta);
            
            // 수렴 검사  
            float residualNorm = MathUtils::VectorNorm(residual, network->nodeCount + network->edgeCount);
...
private:
    ...
    static void CalculateResidualAndJacobian(PneumaticNetwork* network, float* residual, float** jacobian) {
    ...
    static float CalculatePressureDrop(PneumaticEdge* edge, float massFlowRate) {
        if (std::abs(massFlowRate) < 1e-9f) return 0.0f;
        ...
        if (Re < 2300) {
            ...
            float term1 = std::log10(roughness / (3.7f * D) + 5.74f / std::pow(Re, 0.9f));
            f = 0.25f / (term1 * term1);
        }
    ...
    static float CalculatePressureDropDerivative(PneumaticEdge* edge, float massFlowRate) {
        if (std::abs(massFlowRate) < 1e-9f) {
            // 유량이 0에 가까우면 선형 근사
            float A = edge->crossSectionArea * 1e-6f;
            float rho = 1.225f;
            return 2.0f * edge->frictionFactor * edge->length / (edge->diameter * rho * A * A);
        } else {
    ...
    static void SolveLinearSystem(float** A, float* b, float* x, int n) {
        ...
        for (int k = 0; k < n-1; k++) {
            for (int i = k+1; i < n; i++) {
                if (std::abs(A[k][k]) < 1e-12f) continue; // 특이점 방지
    ...
    static void UpdateSolution(PneumaticNetwork* network, float* delta) {
        ...
        for (int i = 0; i < network->nodeCount; i++) {
            if (!network->nodes[i].isPressureSource) {
                ...
                network->nodes[i].gaugePressure = network->pressureVector[i] - network->atmosphericPressure;
            }
        }
    ...
    static void CalculateDerivedQuantities(PneumaticNetwork* network) {
        for (int i = 0; i < network->nodeCount; i++) {
            ...
            node->density = node->pressure / (network->gasConstant * node->temperature);
            
            // 음속 계산: c = √(γRT)
            node->soundSpeed = std::sqrt(network->specificHeatRatio * network->gasConstant * node->temperature);
        ...
        for (int e = 0; e < network->edgeCount; e++) {
            ...
            edge->volumeFlowRate = edge->massFlowRate / rho_std * 60000.0f; // m³/s → L/min
...
public:
    ...
    static int SolveMechanicalSystem(MechanicalSystem* system, float deltaTime) {
        (void)deltaTime; // 현재 버전에서는 정적 상태 해석만 수행, 나중에 룽게-쿠타 적분에서 사용할 예정
        if (!system || system->nodeCount == 0) return -1;
        ...
        for (int i = 0; i < 2 * dof; i++) {
            state[i] += deltaTime / 6.0f * (k1[i] + 2.0f * k2[i] + 2.0f * k3[i] + k4[i]);
        }
...
private:
    ...
    static void UpdateSystemMatrices(MechanicalSystem* system) {
        ...
        for (int n = 0; n < system->nodeCount; n++) {
            ...
            // 회전 관성 (θx, θy, θz)  
            for (int i = 0; i < 3; i++) {
                system->massMatrix[base_idx + 3 + i][base_idx + 3 + i] = node->momentOfInertia[i];
            }
        }
        
        // 댐핑/강성 행렬 (엣지 기여분)
        for (int e = 0; e < system->edgeCount; e++) {
            if (!system->edges[e].base.isActive) continue;
            
            MechanicalEdge* edge = &system->edges[e];
            int node1 = edge->base.fromNodeId;
            int node2 = edge->base.toNodeId;
            
            if (node1 >= 0 && node1 < system->nodeCount && 
                node2 >= 0 && node2 < system->nodeCount) {
                ...
                for (int dof = 0; dof < 6; dof++) {
                    float k = edge->springConstant[dof];
                    float c = edge->dampingCoefficient[dof];
                    
                    // 대각 항목 (자체 강성/댐핑)
                    system->stiffnessMatrix[idx1 + dof][idx1 + dof] += k;
                    system->stiffnessMatrix[idx2 + dof][idx2 + dof] += k;
                    system->dampingMatrix[idx1 + dof][idx1 + dof] += c;
                    system->dampingMatrix[idx2 + dof][idx2 + dof] += c;
                    
                    // 비대각 항목 (결합 강성/댐핑)
                    system->stiffnessMatrix[idx1 + dof][idx2 + dof] -= k;
                    system->stiffnessMatrix[idx2 + dof][idx1 + dof] -= k;
                    system->dampingMatrix[idx1 + dof][idx2 + dof] -= c;
                    system->dampingMatrix[idx2 + dof][idx1 + dof] -= c;
                }
            }
        }
    }
    ...
    static void CalculateStateDerivative(MechanicalSystem* system, const float* state, float* derivative) {
        int dof = 6 * system->nodeCount;
        ...
        // K * q 계산
        MathUtils::MatrixVectorMultiply(system->stiffnessMatrix, state, Kq, dof, dof);
        
        // C * q̇ 계산
        MathUtils::MatrixVectorMultiply(system->dampingMatrix, &state[dof], Cqdot, dof, dof);
    ...
    static void CalculateEdgeForces(MechanicalSystem* system, MechanicalEdge* edge) {
        int node1 = edge->base.fromNodeId;
        int node2 = edge->base.toNodeId;
        
        if (node1 < 0 || node1 >= system->nodeCount || 
            node2 < 0 || node2 >= system->nodeCount) return;
    ...
    static void SolveMassMatrixSystem(MechanicalSystem* system, const float* rhs, float* acceleration) {
        int dof = 6 * system->nodeCount;
    ...
    static void CheckSystemStability(MechanicalSystem* system) {
        ...
        for (int n = 0; n < system->nodeCount; n++) {
            ...
            // 위치/속도가 비정상적으로 큰지 체크
            for (int i = 0; i < 3; i++) {
                if (std::abs(node->position[i]) > 1e6f || std::abs(node->velocity[i]) > 1e6f) {
                    system->isStable = false;
                    std::cout << "Warning: System instability detected at node " << n << std::endl;
