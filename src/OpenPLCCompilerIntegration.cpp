...
#include "LadderIR.h"         // 🔥 **NEW**: LadderIR 지원
#include "LadderToLDConverter.h" // 🔥 **NEW**: IR 기반 변환기
#include "ProgrammingMode.h"  // 🔥 **NEW**: LadderProgram 타입
#include <fstream>
#include <sstream>
#include <iostream>
#include <algorithm>
...
OpenPLCCompilerIntegration::CompileLDString(const std::string& ldContent) {
    ...
    DebugLog("Parsed " + std::to_string(m_instructions.size()) + " instructions");
    DebugLog("Found " + std::to_string(m_variables.size()) + " variables");
    
    // 2. C++ 코드 생성
    result.generatedCode = GenerateCPPCode(m_instructions);
    ...
    // 3. 결과 설정
    result.success = true;
    result.inputCount = m_inputCount;
    result.outputCount = m_outputCount;
    result.memoryCount = m_memoryCount;
    
    DebugLog("Compilation successful!");
    DebugLog("Generated " + std::to_string(result.generatedCode.length()) + " characters of C++ code");
...
void OpenPLCCompilerIntegration::SetIOConfiguration(int inputs, int outputs) {
    ...
    DebugLog("I/O Configuration: " + std::to_string(inputs) + " inputs, " + std::to_string(outputs) + " outputs");
}

bool OpenPLCCompilerIntegration::SaveGeneratedCode(const CompilationResult& result, const std::string& outputPath) {
    if (!result.success) {
...
bool OpenPLCCompilerIntegration::ParseLDContent(const std::string& ldContent) {
    ...
    if (programStart != std::string::npos) {
        ...
        std::string programSection = content.substr(programStart, programEnd - programStart);
...
bool OpenPLCCompilerIntegration::ParseVariableDeclarations(const std::string& content) {
    ...
    while (std::getline(stream, line)) {
        ...
        std::regex varRegex(R"((\w+)\s+AT\s+%([IQMX])([XW]?)(\d+)\.?(\d*)\s*:\s*(\w+);?)");
        std::smatch match;
...
bool OpenPLCCompilerIntegration::ParseInstructions(const std::string& content) {
    ...
    while (std::getline(stream, line)) {
        ...
        if (line.empty() || line[0] == '(' || line.find("PROGRAM") != std::string::npos || 
        ...
        std::string tok;
        while (lineStream >> tok) tokens.push_back(tok);
        ...
        if (instr == "LD" || instr == "AND" || instr == "OR") {
            ...
            if (instr == "LD") {
                ...
                if (hasNot) {
                    if (tokens.size() < 3) continue; // NOT 뒤 피연산자 없음
            ...
            if (instr == "ST") ldInst.type = LDInstruction::ST;
            else if (instr == "S") ldInst.type = LDInstruction::S;
            else ldInst.type = LDInstruction::R;
            ldInst.operand = tokens[1];
        } else if (instr == "TON") {
            // TON <Tn> <Kxxx>
            if (tokens.size() < 2) continue;
            ...
            ldInst.operand = tokens[1];
            if (tokens.size() >= 3) ldInst.preset = tokens[2];
        } else if (instr == "TOF" || instr == "CTU" || instr == "CTD") {
            // 향후 확장용: 기본 피연산자만 수용
            ldInst.type = StringToInstructionType(instr);
            if (tokens.size() >= 2) ldInst.operand = tokens[1];
        } else {
            // 알 수 없는 명령: 건너뜀
            continue;
        }

        m_instructions.push_back(ldInst);
        DebugLog("Line " + std::to_string(lineNumber) + ": " + instr + (ldInst.operand.empty()?"":" ") + ldInst.operand);
    }
    
    return true;
}

std::string OpenPLCCompilerIntegration::GenerateCPPCode(const std::vector<LDInstruction>& instructions) {
...
std::string OpenPLCCompilerIntegration::GenerateFunctionHeader() {
    ...
    header << "// Generated by FX3U PLC Simulator - OpenPLC Compiler Integration\n";
    header << "// Target: FX3U-32M (16 inputs, 16 outputs)\n";
    header << "#include <iostream>\n";
    header << "#include <chrono>\n\n";
    
    header << "// PLC Logic Execution Function\n";
    header << "extern \"C\" void ExecutePLCLogic(bool* X, bool* Y, bool* M, int* T, int* C) {\n";
...
std::string OpenPLCCompilerIntegration::GenerateVariableDeclarations() {
    ...
    vars << "    static auto last_scan_time = std::chrono::steady_clock::now();\n";
    vars << "    auto current_time = std::chrono::steady_clock::now();\n";
    vars << "    auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(current_time - last_scan_time).count();\n";
    vars << "    \n";
...
std::string OpenPLCCompilerIntegration::GenerateIOMapping() {
    ...
    mapping << "    // Inputs: X0-X" << (m_inputCount-1) << "\n";
    mapping << "    // Outputs: Y0-Y" << (m_outputCount-1) << "\n";
    mapping << "    // Memory: M0-M" << (m_memoryCount-1) << "\n";
    mapping << "    \n";
    
    return mapping.str();
}

std::string OpenPLCCompilerIntegration::GenerateExecutionCode(const std::vector<LDInstruction>& instructions) {
    std::stringstream code;
...
std::string OpenPLCCompilerIntegration::TranslateInstruction(const LDInstruction& instruction) {
    switch (instruction.type) {
        ...
        case LDInstruction::TON: {
            ...
            return "/* TON " + timerVar + " " + instruction.preset + " */ " +
                   "if (accumulator && T[" + timerVar.substr(1) + "] < " + presetValue + ") " +
                   "T[" + timerVar.substr(1) + "] += elapsed_ms; " +
                   "accumulator = (T[" + timerVar.substr(1) + "] >= " + presetValue + ");";
        }
        
        default:
            return "// Unknown instruction: " + std::to_string((int)instruction.type);
    }
}

OpenPLCCompilerIntegration::LDInstruction::Type 
OpenPLCCompilerIntegration::StringToInstructionType(const std::string& typeStr) {
    std::string upper = typeStr;
    std::transform(upper.begin(), upper.end(), upper.begin(), ::toupper);
    
    if (upper == "LD") return LDInstruction::LD;
    if (upper == "LDN" || upper == "LD NOT") return LDInstruction::LDN;
    if (upper == "AND") return LDInstruction::AND;
    if (upper == "ANDN" || upper == "AND NOT") return LDInstruction::ANDN;
    if (upper == "OR") return LDInstruction::OR;
    if (upper == "ORN" || upper == "OR NOT") return LDInstruction::ORN;
    if (upper == "ST") return LDInstruction::ST;
    if (upper == "S") return LDInstruction::S;
    if (upper == "R") return LDInstruction::R;
    if (upper == "TON") return LDInstruction::TON;
    if (upper == "TOF") return LDInstruction::TOF;
    if (upper == "CTU") return LDInstruction::CTU;
    if (upper == "CTD") return LDInstruction::CTD;
...
OpenPLCCompilerIntegration::CompilationResult 
OpenPLCCompilerIntegration::CompileLadderProgramWithIR(const LadderProgram& ladderProgram) {
...
OpenPLCCompilerIntegration::CompileIRProgram(const LadderIRProgram& irProgram) {
    ...
    try {
        ...
        DebugLog("Generated " + std::to_string(stackInstructions.size()) + " stack instructions");
        ...
        if (result.success) {
            ...
            result.intermediateCode += "// Total IR rungs: " + std::to_string(irProgram.GetRungCount()) + "\n";
            result.intermediateCode += "// Stack instructions: " + std::to_string(stackInstructions.size()) + "\n";
...
std::string OpenPLCCompilerIntegration::GenerateOpenPLCHeader() {
    ...
    header << "// Optimization Level: " << m_optimizationLevel << "\n";
    header << "// I/O Configuration: " << m_inputCount << " inputs, " << m_outputCount << " outputs\n";
    header << "\n";
