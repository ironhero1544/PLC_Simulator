...
void Application::SimulatePneumatic() {
  ...
  for (const auto& wire : wires_) {
    if (!wire.isElectric) {
      pneumaticAdjacencyList[std::make_pair(wire.fromComponentId,
                                            wire.fromPortId)]
          .push_back(std::make_pair(wire.toComponentId, wire.toPortId));
      pneumaticAdjacencyList[std::make_pair(wire.toComponentId, wire.toPortId)]
          .push_back(std::make_pair(wire.fromComponentId, wire.fromPortId));
    }
  }
  ...
  for (const auto& comp : placed_components_) {
    if (comp.type == ComponentType::VALVE_SINGLE) {
      ...
      if (sol_a_active) {
        pneumaticAdjacencyList[p].push_back(a);
        pneumaticAdjacencyList[a].push_back(p);
      } else {
        pneumaticAdjacencyList[p].push_back(b);
        pneumaticAdjacencyList[b].push_back(p);
      }
    } else if (comp.type == ComponentType::VALVE_DOUBLE) {
      float last_active = comp.internalStates.count("last_active_solenoid")
                              ? comp.internalStates.at("last_active_solenoid")
                              : 0.0f;
  ...
  for (const auto& comp : placed_components_) {
    if (comp.type == ComponentType::FRL) {
      ...
      float airPressure = comp.internalStates.count("air_pressure")
    ...
    if (pneumaticAdjacencyList.count(current)) {
      for (const auto& neighbor : pneumaticAdjacencyList.at(current)) {
        if (processedPorts.find(neighbor) == processedPorts.end()) {
          port_pressures_[neighbor] =
              current_pressure > 0.1f ? current_pressure - 0.1f : 0.0f;
