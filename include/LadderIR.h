 ...
 * 
 * 이 파일은 UI 래더 다이어그램과 PLC 스택 머신 코드 사이의 중간 표현을 정의합니다.
 * 병렬(OR) 논리 회로의 정확한 변환을 위해 설계되었습니다.
 * 
...
struct IRConnection {
    ...
    IRConnection(size_t from, size_t to, int outPort = 0, int inPort = 0)
        : fromNodeId(from), toNodeId(to), outputPort(outPort), inputPort(inPort) {}
};
...
public:
    ...
    IRNode(IRNodeType type, size_t id, const std::string& address = "")
        : nodeType(type), nodeId(id), deviceAddress(address), 
          rungIndex(-1), cellIndex(-1) {}
...
public:
    ...
    IRTimerNode(size_t id, const std::string& address, float value = 1.0f)
        : IRNode(IRNodeType::TIMER_ON, id, address), timeValue(value), timeUnit("s") {}
};
...
public:
    ...
    IROrBlockNode(size_t id) 
        : IRNode(IRNodeType::OR_BLOCK, id) {}
...
struct IRRung {
    ...
    // 연결 추가
    void AddConnection(size_t fromId, size_t toId, int outPort = 0, int inPort = 0);
...
private:
    std::vector<IRRung> m_rungs;                      ///< 모든 룽들
    std::map<std::string, size_t> m_deviceNodeMap;    ///< 디바이스 주소 → 노드 ID 매핑
    size_t m_nextNodeId;                              ///< 다음 노드 ID
    
public:
    LadderIRProgram() : m_nextNodeId(1) {}  // 0은 전원선용 예약
    
    // 룽 관리
    void AddRung(IRRung&& rung);
    size_t GetRungCount() const { return m_rungs.size(); }
    IRRung& GetRung(size_t index) { return m_rungs[index]; }
    const IRRung& GetRung(size_t index) const { return m_rungs[index]; }
    
    // 노드 ID 관리
    size_t GenerateNodeId() { return m_nextNodeId++; }
...
private:
    ...
    // OR 블록 스택 명령어 생성
    static std::vector<std::string> ConvertOrBlockToStack(const IROrBlockNode& orBlock);
