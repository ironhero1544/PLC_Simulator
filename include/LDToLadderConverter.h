...
public:
     ...
     * @return 변환 성공 여부
     */
    bool ConvertFromLDFile(const std::string& ldFilePath, LadderProgram& ladderProgram);
     ...
     * @return 변환 성공 여부
     */
    bool ConvertFromLDString(const std::string& ldContent, LadderProgram& ladderProgram);
    ...
    const ConversionStats& GetConversionStats() const { return m_stats; }

private:
    bool m_debugMode;
    std::string m_lastError;
    ConversionStats m_stats;
    ...
    std::vector<LDNetwork> m_parsedNetworks;
    ...
    LadderInstructionType ConvertLDTypeToInstruction(const std::string& ldType, const std::string& operation);
    ...
    std::vector<std::string> ExtractXMLTags(const std::string& xml, const std::string& tagName);
    std::string GetXMLAttribute(const std::string& xmlTag, const std::string& attrName);
