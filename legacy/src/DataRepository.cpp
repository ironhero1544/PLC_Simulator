...
bool WiringRepo_RemoveWire(WiringRepository* repo, int wireId) {
    ...
    auto it = std::find_if(repo->wires->begin(), repo->wires->end(),
        [wireId](const Wire& wire) {
...
LadderRepository* CreateLadderRepository(plc::LadderProgram* program,
    ...
    if (!program || !deviceStates || !timerStates || !counterStates) {
        std::cerr << "Error: LadderRepository parameters cannot be null" << std::endl;
        return nullptr;
    }
...
plc::Rung* LadderRepo_GetRung(LadderRepository* repo, int rungIndex) {
    ...
    if (rungIndex >= 0 && rungIndex < static_cast<int>(repo->program->rungs.size())) {
