...
bool LDToLadderConverter::ConvertFromLDFile(const std::string& ldFilePath, LadderProgram& ladderProgram) {
    // 파일 읽기
    ...
    LogDebug("📖 Reading .ld file: " + ldFilePath + " (" + std::to_string(ldContent.length()) + " bytes)");
    
    return ConvertFromLDString(ldContent, ladderProgram);
}

bool LDToLadderConverter::ConvertFromLDString(const std::string& ldContent, LadderProgram& ladderProgram) {
    // 통계 초기화
    m_stats = ConversionStats();
    m_parsedNetworks.clear();
    ...
    LogDebug("📊 Stats: " + std::to_string(m_stats.networksCount) + " networks, " + 
             std::to_string(m_stats.contactsCount) + " contacts, " + 
             std::to_string(m_stats.coilsCount) + " coils");
...
bool LDToLadderConverter::ParseLDFile(const std::string& ldContent) {
    ...
    // 네트워크 섹션들 추출
    std::vector<std::string> networkSections = ExtractXMLTags(ldContent, "network");
    ...
    for (size_t i = 0; i < networkSections.size(); ++i) {
        ...
        m_parsedNetworks.push_back(network);
        m_stats.networksCount++;
    }
    
    return true;
}

bool LDToLadderConverter::ParseNetwork(const std::string& networkXML, LDNetwork& network) {
    ...
    // 연결선 정보 추출
    std::vector<std::string> connections = ExtractXMLTags(networkXML, "connection");
    for (const auto& conn : connections) {
        network.connections.push_back(conn);
        m_stats.connectionsCount++;
    }
...
bool LDToLadderConverter::ParseElement(const std::string& elementXML, LDElement& element) {
    // 기본 속성 추출
    ...
    if (!xStr.empty()) element.x = std::stoi(xStr);
    if (!yStr.empty()) element.y = std::stoi(yStr);
    ...
    LogDebug("🔧 Parsed element: " + element.name + " (" + element.operation + ") at (" + 
             std::to_string(element.x) + "," + std::to_string(element.y) + ")");
    
    return !element.name.empty();
}

bool LDToLadderConverter::ConvertNetworksToLadder(LadderProgram& ladderProgram) {
    ...
    // 각 네트워크를 룽으로 변환
    for (const auto& network : m_parsedNetworks) {
        Rung rung;
        rung.number = network.networkNumber;
        
        if (!ConvertNetworkToRung(network, rung)) {
            SetError("Failed to convert network " + std::to_string(network.networkNumber) + " to rung");
            return false;
        }
    ...
    LogDebug("✅ Converted to " + std::to_string(ladderProgram.rungs.size()) + " rungs with " + 
             std::to_string(ladderProgram.verticalConnections.size()) + " vertical connections");
    
    return true;
}

bool LDToLadderConverter::ConvertNetworkToRung(const LDNetwork& network, Rung& rung) {
    LogDebug("🔄 Converting network " + std::to_string(network.networkNumber) + " to rung");
    ...
    std::sort(sortedElements.begin(), sortedElements.end(), 
              [](const LDElement& a, const LDElement& b) {
                  return a.x < b.x;
              });
    ...
    for (const auto& element : sortedElements) {
        ...
        instruction.type = ConvertLDTypeToInstruction(element.type, element.operation);
        instruction.address = element.name;
        
        // 특별한 속성들 처리
        if (element.attributes.count("negated") && element.attributes.at("negated") == "true") {
            // 부정 접점 처리
...
LadderInstructionType LDToLadderConverter::ConvertLDTypeToInstruction(const std::string& ldType, const std::string& operation) {
    if (ldType == "contact") {
    ...
    LogDebug("⚠️ Unknown LD type/operation: " + ldType + "/" + operation + " -> EMPTY");
...
void LDToLadderConverter::OptimizeRungLayout(Rung& rung) {
    ...
    // 비어있지 않은 명령어들을 다시 배치
    for (size_t i = 0; i < nonEmptyInstructions.size() && i < rung.cells.size(); ++i) {
        rung.cells[i] = nonEmptyInstructions[i];
    }
}

void LDToLadderConverter::GenerateVerticalConnections(LadderProgram& ladderProgram) {
...
std::string LDToLadderConverter::ExtractXMLTag(const std::string& xml, const std::string& tagName) {
...
std::vector<std::string> LDToLadderConverter::ExtractXMLTags(const std::string& xml, const std::string& tagName) {
...
std::string LDToLadderConverter::GetXMLAttribute(const std::string& xmlTag, const std::string& attrName) {
...
bool LDToLadderConverter::ValidateConvertedLadder(const LadderProgram& ladderProgram) {
...
bool LDToLadderConverter::CheckInstructionValidity(const LadderInstruction& instruction) {
    ...
    if (instruction.type == LadderInstructionType::XIC ||
        ...
        if (instruction.address.empty()) {
            LogDebug("⚠️ Instruction without address: type " + std::to_string(static_cast<int>(instruction.type)));
            return false;
        }
    }
