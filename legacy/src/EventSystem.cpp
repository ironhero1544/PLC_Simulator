...
bool EventDispatcher_Subscribe(EventDispatcher* dispatcher, EventType type, EventCallback callback, void* userData) {
...
bool EventDispatcher_Unsubscribe(EventDispatcher* dispatcher, EventType type, EventCallback callback) {
    if (!dispatcher || !callback || type < 0 || type >= EVENT_TYPE_COUNT) {
        return false;
    }
...
void EventDispatcher_Dispatch(EventDispatcher* dispatcher, const EventData* eventData) {
...
EventData CreateWireRemovedEvent(int wireId, int fromComponent, int toComponent) {
    EventData event;
...
EventData CreateIOMappingUpdatedEvent(int mappingCount, int inputCount, int outputCount, bool success, void* mappingPtr) {
    EventData event;
...
const char* EventTypeToString(EventType type) {
    switch (type) {
        case EVENT_DATA_CHANGED: return "DATA_CHANGED";
        case EVENT_MODE_CHANGED: return "MODE_CHANGED";
        case EVENT_COMPONENT_ADDED: return "COMPONENT_ADDED";
        case EVENT_COMPONENT_REMOVED: return "COMPONENT_REMOVED";
        case EVENT_WIRE_ADDED: return "WIRE_ADDED";
        case EVENT_WIRE_REMOVED: return "WIRE_REMOVED";
        case EVENT_PLC_STATE_CHANGED: return "PLC_STATE_CHANGED";
        case EVENT_IO_MAPPING_UPDATED: return "IO_MAPPING_UPDATED";
...
void PrintEventData(const EventData* eventData) {
    ...
    switch (eventData->type) {
        ...
        case EVENT_PLC_STATE_CHANGED:
            std::cout << " (Running: " << (eventData->data.plcStateEvent.isRunning ? "true" : "false") << ")";
            break;
        case EVENT_IO_MAPPING_UPDATED:
            std::cout << " (Mappings: " << eventData->data.ioMappingEvent.mappingCount 
                     ...
                     << ", Success: " << (eventData->data.ioMappingEvent.success ? "true" : "false") << ")";
            break;
        default:
            break;
    }
