...
public:
     ...
     * @return 로드 성공 여부
     */
    bool LoadFromCompilationResult(const OpenPLCCompilerIntegration::CompilationResult& result);
     ...
     * @return 실행 통계
     */
    const ExecutionResult& GetLastExecutionResult() const { return m_lastResult; }

    /**
     * @brief PLC 메모리 직접 접근 (디버깅용)
     * @return 메모리 참조
     */
    const PLCMemory& GetMemory() const { return m_memory; }
...
private:
    ...
    std::string m_loadedCode;
    bool m_debugMode = false;
    bool m_isRunning = false;
    bool m_continuousMode = false;
    int m_cycleTime_ms = 65;
