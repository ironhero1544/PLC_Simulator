...
struct ComponentMapping {
    ...
    ComponentMapping()
        : componentId(-1), componentPortId(-1), plcAddress(""), 
          componentType(ComponentType::PLC), componentRole("") {}
          
    ComponentMapping(int compId, int portId, const std::string& addr, 
                     ComponentType type, const std::string& role)
        : componentId(compId), componentPortId(portId), plcAddress(addr), 
          componentType(type), componentRole(role) {}
};
...
struct IOMapping {
    ...
    IOMapping() 
        : plcComponentId(-1), isValid(false), inputCount(0), outputCount(0), lastUpdated(0) {
        mappings.clear();
        plcPorts.clear();
    }
    ...
    void AddMapping(const ComponentMapping& mapping) {
        ...
        if (mapping.plcAddress.length() > 0) {
            if (mapping.plcAddress[0] == 'X') inputCount++;
            else if (mapping.plcAddress[0] == 'Y') outputCount++;
        }
    }
    ...
    bool ValidateMapping() const {
        if (plcComponentId == -1) return false;
...
struct ConnectionTrace {
    ...
    ConnectionTrace()
        : fromComponentId(-1), fromPortId(-1), toComponentId(-1), toPortId(-1), isElectrical(true) {
        wireIds.clear();
    }
};
