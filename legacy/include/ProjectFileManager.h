...
public:
     ...
     * @return 저장 결과
     */
    SaveResult SaveProject(const LadderProgram& program, const std::string& filePath, 
...
private:
    ...
    std::unique_ptr<XMLSerializer> m_xmlSerializer;
    ...
    std::string CreateSimpleJSON(const std::map<std::string, std::string>& keyValues);
    std::map<std::string, std::string> ParseSimpleJSON(const std::string& jsonContent);
};
