...
private:
    ...
    std::string SerializeCell(const LadderInstruction& cell, int rungIndex, int cellIndex) const;
    std::string SerializeVerticalConnections(const std::vector<VerticalConnection>& connections) const;
    std::string SerializeVerticalConnection(const VerticalConnection& conn, int index) const;

    // XML 파싱 관련 함수들
    bool ParseXMLHeader(const std::string& xmlContent);
    bool ParseMetadata(const std::string& xmlContent, DeserializationResult& result);
    ...
    bool ParseVerticalConnections(const std::string& xmlContent, LadderProgram& program);
    bool ParseVerticalConnection(const std::string& connXML, VerticalConnection& conn);
    ...
    // XML 파싱 헬퍼들
    std::string ExtractXMLTag(const std::string& xml, const std::string& tagName) const;
    std::vector<std::string> ExtractXMLTags(const std::string& xml, const std::string& tagName) const;
    std::string GetXMLAttribute(const std::string& xmlTag, const std::string& attrName) const;
